      ...
      builder: (context, child) {
        return MaterialApp(
          ...
          builder: (context, widget) {
            ...
            return MediaQuery(data: MediaQuery.of(context).copyWith(textScaler: TextScaler.linear(appState.textScale)), child: app);
          },
          theme: ThemeData(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Spacer(),
              ElevatedButton.icon(onPressed: () => _loginAndGo(context, UserIdentity.hgBoundUser), icon: const Icon(Icons.flash_on, size: 20), label: const Text('HAPPY GO 快速登入 (推薦)', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)), style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFF59E0B), foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), elevation: 0)),
              const SizedBox(height: 12),
              ElevatedButton.icon(onPressed: () => _loginAndGo(context, UserIdentity.newPhoneUser), icon: const Icon(Icons.phone_android, size: 20), label: const Text('手機號碼註冊 / 登入'), style: ElevatedButton.styleFrom(backgroundColor: Colors.grey[100], foregroundColor: const Color(0xFF1F2937), padding: const EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), elevation: 0)),
              const SizedBox(height: 16),
              TextButton(onPressed: () => _loginAndGo(context, UserIdentity.guest), child: const Text('先逛逛，稍後再註冊', style: TextStyle(color: Colors.grey))),
...
class GodModeFab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Positioned(
      left: 16, bottom: 16,
      child: FloatingActionButton.small(heroTag: 'god_mode', backgroundColor: const Color(0xFF1F2937), foregroundColor: Colors.amber, onPressed: () => _showConsole(context), child: const Icon(Icons.tune)),
    );
  }
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              ListTile(
                contentPadding: EdgeInsets.zero, leading: const Icon(Icons.format_size, color: darkTeal),
                title: const Text('樂齡大字模式', style: TextStyle(fontWeight: FontWeight.bold, color: darkTeal)),
                trailing: Switch(value: appState.textScale > 1.0, activeColor: primaryEmerald, onChanged: (v) { appState.toggleTextScale(); Navigator.pop(context); }),
              ),
              const SizedBox(height: 16),
...
class HomePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [IconButton(icon: const Icon(Icons.notifications_outlined), onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const NotificationPage())))],
      ),
      body: ListView(
        ...
        children: [
          ...
          const Row(children: [Icon(Icons.local_hospital, color: Colors.blue), SizedBox(width: 8), Text('亞東醫院 衛教專區', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF1F2937)))]),
          const SizedBox(height: 12),
  ...
  Widget _buildHeroStatusCard(BuildContext context) {
    ...
    return Container(
      padding: const EdgeInsets.all(24),
      decoration: BoxDecoration(gradient: const LinearGradient(colors: [primaryEmerald, darkTeal], begin: Alignment.topLeft, end: Alignment.bottomRight), borderRadius: BorderRadius.circular(24), boxShadow: [BoxShadow(color: primaryEmerald.withOpacity(0.3), blurRadius: 15, offset: const Offset(0, 8))]),
      child: Column(
              ...
              Column(
                ...
                children: [
                  ...
                  Row(children: [
                    ...
                    if (isBound) ...[const SizedBox(width: 8), Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: const Color(0xFFF59E0B), borderRadius: BorderRadius.circular(12)), child: const Text('HG 已綁定', style: TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.bold)))]
                  ]),
                ],
              ),
  ...
  Widget _buildRadarAndAiCard(BuildContext context) {
    ...
    switch(appState.currentScenario) {
      case AiScenario.mobilityDecline: themeColor = Colors.orange; aiTitle = "健康守護提案"; aiMsg = "系統觀察到您近期的『行動力』指標有下降趨勢。為保護關節健康，我為您爭取到【大樹藥局 - 葡萄糖胺】專屬優惠，以及自費復健評估專案，請問需要安排嗎？"; btn1Txt = "用 500 點換購葡萄糖胺"; btn1Icon = Icons.shopping_cart; btn2Txt = "預約亞東自費復健"; btn2Icon = Icons.calendar_month; break;
      case AiScenario.vitalsWarning: themeColor = Colors.redAccent; aiTitle = "防護力異常警示"; aiMsg = "您近期的防護力數值出現異常波動。為了您的健康，建議您進一步了解【亞東醫院高階腦部 MRI 健檢專案】，及早發現及早預防。"; btn1Txt = "了解高階健檢專案"; btn1Icon = Icons.medical_services; btn2Txt = "重新進行檢測"; btn2Icon = Icons.refresh; break;
      case AiScenario.perfectConsistency: themeColor = primaryEmerald; aiTitle = "極致健康解鎖"; aiMsg = "太棒了！您已【連續 30 天達標】，且三大健康力超越 95% 用戶！\n\nAI 已為您解鎖隱藏福利：購買【南山人壽外溢保單】首年保費減免 10%，同時桂格送您專屬禮物！"; btn1Txt = "領取保單 10% 減免憑證"; btn1Icon = Icons.shield; btn2Txt = "領取桂格贊助兌換券"; btn2Icon = Icons.card_giftcard; break;
    }
    return Container(
      ...
      child: Column(
        children: [
          Container(
            ...
            child: Row(
              children: [
                SizedBox(width: 110, height: 110, child: CustomPaint(painter: TriangleRadarPainter(stats: appState.radar3D))), const SizedBox(width: 20),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  _buildRadarStatBar('🧠 腦動力', appState.radar3D['腦動力']!, Colors.amber.shade500), _buildRadarStatBar('🏃‍♂️ 行動力', appState.radar3D['行動力']!, Colors.blue.shade400), _buildRadarStatBar('🛡️ 防護力', appState.radar3D['防護力']!, primaryEmerald),
                ]))
              ],
            ),
          ),
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 12), Text(aiMsg, style: const TextStyle(height: 1.6, color: Color(0xFF4B5563), fontSize: 13)), const SizedBox(height: 16),
  ...
  Widget _buildTrendCard(BuildContext context) {
    return Container(
      ...
      child: ListTile(
        ...
        onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const HealthDataPage())),
      ),
    );
  }
...
class FamilyPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('家人群組與關懷')),
      body: ListView(
        padding: const EdgeInsets.all(20),
        children: [
          Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.orange.shade50, borderRadius: BorderRadius.circular(16), border: Border.all(color: Colors.orange.shade100)), child: const Row(children: [Icon(Icons.volunteer_activism, color: Colors.orange), SizedBox(width: 12), Expanded(child: Text('將您的健康點「轉贈」給子孫，或透過邀請連結關懷父母的健康狀況。', style: TextStyle(color: Color(0xFF92400E), fontSize: 13, height: 1.5)))])),
          const SizedBox(height: 24),
          const Text('我邀請的家人', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Color(0xFF1F2937))), const SizedBox(height: 12),
          _buildFamilyMemberCard(context, '大兒子', '已連線', true), _buildFamilyMemberCard(context, '小女兒', '已連線', true),
          const SizedBox(height: 12),
          OutlinedButton.icon(onPressed: (){}, icon: const Icon(Icons.person_add), label: const Text('產生邀請連結 (Line/簡訊)'), style: OutlinedButton.styleFrom(foregroundColor: darkTeal, side: const BorderSide(color: darkTeal), padding: const EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)))),
          const SizedBox(height: 32),
          const Text('邀請我的家人 (長輩)', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Color(0xFF1F2937))), const SizedBox(height: 12),
          Container(decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), border: Border.all(color: Colors.grey.shade200)), child: ListTile(contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), leading: CircleAvatar(backgroundColor: Colors.blue.shade100, child: Icon(Icons.person, color: Colors.blue.shade700)), title: const Text('爸爸', style: TextStyle(fontWeight: FontWeight.bold)), subtitle: const Text('今日已完成 2 項任務', style: TextStyle(color: primaryEmerald, fontSize: 12)), trailing: TextButton(onPressed: (){}, child: const Text('查看狀態')))),
        ],
      ),
    );
  }
  ...
  Widget _buildFamilyMemberCard(BuildContext context, String name, String status, bool canGift) {
    return Container(
      ...
      child: ListTile(
        ...
        leading: CircleAvatar(backgroundColor: primaryEmerald.withOpacity(0.2), child: const Icon(Icons.person, color: darkTeal)),
        title: Text(name, style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF1F2937))), subtitle: Text(status, style: const TextStyle(color: Colors.grey, fontSize: 12)),
        trailing: canGift ? ElevatedButton.icon(onPressed: () => _showGiftDialog(context, name), icon: const Icon(Icons.card_giftcard, size: 16), label: const Text('轉贈點數'), style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFF59E0B), foregroundColor: Colors.white, elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)))) : null,
      ),
    );
  }

  void _showGiftDialog(BuildContext context, String name) {
    showDialog(context: context, builder: (c) => AlertDialog(backgroundColor: Colors.white, title: Text('轉贈健康點給 $name'), content: const TextField(decoration: InputDecoration(labelText: '輸入轉贈點數', suffixText: 'Pts', border: OutlineInputBorder()), keyboardType: TextInputType.number), actions: [TextButton(onPressed: () => Navigator.pop(c), child: const Text('取消', style: TextStyle(color: Colors.grey))), ElevatedButton(onPressed: () { Navigator.pop(c); ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('成功轉贈給 $name！'))); }, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFF59E0B), foregroundColor: Colors.white), child: const Text('確認轉贈'))]));
  }
}
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildSettingsList([
            ...
            _buildListTile(context, Icons.fact_check_outlined, '歷史任務與測驗紀錄', '您完成的互動任務軌跡', const SubPage(title: '歷史任務紀錄')),
            _buildListTile(context, Icons.monetization_on_outlined, '健康點數累兌明細', '點數獲得與兌換紀錄', const SubPage(title: '點數明細')),
          ]),
          _buildSectionTitle('帳號與安全'),
          _buildSettingsList([
            _buildListTile(context, Icons.manage_accounts_outlined, '個人資料設定', '修改手機、地址等資訊', const SubPage(title: '個人資料設定')),
            _buildListTile(context, Icons.security_outlined, '隱私與數據授權', '管理您的個人化推薦同意狀態', const SubPage(title: '隱私設定')),
          ]),
          _buildSectionTitle('關於本服務'),
          _buildSettingsList([
            _buildListTile(context, Icons.description_outlined, '服務條款與政策須知', null, const SubPage(title: '服務條款')),
            _buildListTile(context, Icons.help_outline, '常見問題與客服中心', null, const SubPage(title: '客服中心')),
          ]),
...
// ==========================================
// 📍 附屬頁面：健康數據追蹤頁 (Health Data Drill-down)
// ==========================================
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildDataCard('血壓', '118 / 75', 'mmHg', '正常', Colors.blue),
          _buildDataCard('靜止心率', '72', 'bpm', '正常', Colors.redAccent),
          _buildDataCard('BMI', '23.4', '', '標準', primaryEmerald),
        ],
      ),
    );
  }
...
class NotificationPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView(
        children: [
          ListTile(leading: const CircleAvatar(backgroundColor: Colors.orange, child: Icon(Icons.warning_amber, color: Colors.white)), title: const Text('AI 健康管家提醒'), subtitle: const Text('您今日的【金幣深蹲王】任務尚未完成喔！'), trailing: const Text('10 分鐘前', style: TextStyle(color: Colors.grey, fontSize: 10)), onTap: (){}),
          const Divider(height: 1),
          ListTile(leading: const CircleAvatar(backgroundColor: primaryEmerald, child: Icon(Icons.volunteer_activism, color: Colors.white)), title: const Text('親友動態'), subtitle: const Text('大兒子已領取您贈送的 50 點健康點！'), trailing: const Text('2 小時前', style: TextStyle(color: Colors.grey, fontSize: 10)), onTap: (){}),
        ],
      ),
    );
  }
...
class SubPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.construction, size: 64, color: Colors.grey.shade300), const SizedBox(height: 16), Text('$title 頁面開發中...', style: const TextStyle(color: Colors.grey))])),
    );
  }
}
